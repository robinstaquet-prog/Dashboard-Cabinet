...
type FormData = Omit<Patient, 'id' | 'createdAt' | 'updatedAt' | 'sessions'>
  ...
  return (
    <div className="p-6 max-w-2xl">
      <div className="flex items-center gap-3 mb-6">
        ...
        <h1 className="text-xl font-semibold text-stone-800">Nouveau patient</h1>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <section className="bg-white rounded-xl border border-stone-200 p-5 space-y-4">
          ...
          <div>
            <label className="block text-sm font-medium text-stone-700 mb-1">Anamnèse</label>
            <textarea
              value={form.anamnese}
              onChange={(e) => set('anamnese', e.target.value)}
              rows={5}
              className="w-full border border-stone-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-teal-500 resize-none"
              placeholder="Antécédents, motif de consultation, histoire de la maladie…"
            />
          </div>
        </section>
